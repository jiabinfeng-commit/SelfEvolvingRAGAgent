...
│   ├── setup_new_machine.sh      ★ 新人/换机器：全本地一键搭建（不依赖任何云主机，默认路径）
│   ├── setup_local.sh            作者本人：**云主机专用，已非默认**——本地开发复用自己云上 PG + 向量库
│   ├── tunnel_pg.sh              **云主机专用，已非默认**——SSH 隧道连云上 PG（需 RAG_CLOUD_HOST）
...
# 默认 PG 用本地 Docker（见 §5.3 方式 1），.env 里 PG_HOST=localhost 即可，不需要隧道
# 只有「你确实有云主机、且想直连云上 PG」才先开隧道（需停本地 rag-pg，或换 RAG_LOCAL_PG_PORT=5433）：
...
| PostgreSQL | 本地 Docker 起一个最简的即可（见下 §5.3 方式 1，默认路径）；复用云上 PG + SSH 隧道是**云主机专用、已非默认**的方式 |
...
# 方式 2（云主机专用，已非默认）：复用云上 PG（不开放公网端口，走 SSH 隧道，最安全）
# ⚠️ 本地已用 Docker 起 rag-pg 占着 5432 时，开隧道前先 docker stop rag-pg（或换 RAG_LOCAL_PG_PORT=5433）
bash scripts/tunnel_pg.sh        # 这个终端要一直开着
```

> **为什么云上 PG 要隧道**：云上 PG 只监听 `127.0.0.1`，公网连不上（这是好事）。隧道在云主机内部转发给 `localhost:5432`，
> 既不用改 PG 配置，也不用开阿里云安全组端口。方式 1（本地 Docker）是默认路径，方式 2 仅当你确实要直连云上数据时才用。
...
最小可用配置（本地 Docker PG + 百炼 LLM；若直连云上 PG 才走隧道，见 §5.3 方式 2）：
