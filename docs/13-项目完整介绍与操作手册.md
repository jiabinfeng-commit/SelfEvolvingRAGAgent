...
│   ├── setup_local.sh            本地环境一键搭建
│   ├── tunnel_pg.sh              SSH 隧道连云上 PG
...
```bash
# 终端 1：后端
cd /Users/fengjiabin/PycharmProjects/Self_Evolving_RAG_Agent
bash scripts/tunnel_pg.sh                  # 若 PG 在云上，先开隧道（保持开着）
...
**依赖清单**（`scripts/setup_local.sh` 里就是这套）：

```bash
pip install pymilvus milvus-lite psycopg2-binary python-dotenv sentence-transformers
# 前端相关
